...
import delaySensitive_gol as gol
from itertools import combinations
...
def plot_phase_transitions_with_bump(data: list[float], lmbda_scale: list[float], transitions: list, rule: str, lower_threshold = 0.05, upper_threshold = 0.95, min_bump = 0.2):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"outputs/delay-sensitive/{rule.replace('/', '-')}/plot")
    # plt.show()
    plt.close()
...
if __name__ == "__main__":
    ...
    usedRules = set()
    delays = [i/20 for i in range(1, 21)]
    taus = [i/20 for i in range(1, 21)]
    ...
    while len(sample_pairs) > 0:
        ...
        dPath = Path(f"outputs/delay-sensitive/{rule_str}/")
        ...
        # Open file for writing delay, tau and density results
        fp = open(f"outputs/delay-sensitive/{rule_str}/dts.txt", "w")
        ...
        for delay in delays:
            for tau in taus:
                ...
                log(f"Running Conway Simulation for delay & tau - {delay} & {tau}")
                conway.simulate(False, False)

                avg_den = conway.getAvgDensity()
                log(f"Calculated Average Density for delay & tau - {delay} & {tau}: {avg_den}")
                fp.write(f"{delay} {tau} {avg_den}\n")
        ...
        # Write to dataset
        fPath = Path("outputs/delay-sensitive/dataset.txt")
        ...
        log("Plotting delay and tau vs avg density")
