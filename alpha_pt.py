...
if __name__ == "__main__":
    ...
    def plot_phase_transitions_with_bump(data: list[float], alpha_scale: list[float], transitions: list, rule: str, lower_threshold = 0.05, upper_threshold = 0.95, min_bump = 1.5):
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(f"outputs/apha/{rule.replace('/', '-')}/plot")
        # plt.show()
        plt.close()
    ...
    # For each possible number of B parameters (1-5)
    for b_count in range(1, 9):
        # Generate all combinations of numbers 1-8, taking b_count at a time
        # This ensures numbers are unique and we can sort them later
        b_combinations = list(combinations(range(1, 9), b_count))

        # For each possible number of S parameters (1-5)
        for s_count in range(1, 9):
            # Generate all combinations of numbers 1-8, taking s_count at a time
            ...
            for b_combo in b_combinations:
                for s_combo in s_combinations:
                    # Format B part - numbers already in ascending order from combinations()
                    b_part = 'B' + ','.join(map(str, b_combo))

                    # Format S part - numbers already in ascending order from combinations()
    ...
    while len(rules) > 0:
        ...
        for alpha in alphas:
            ...
            conway = gol.GameOfLife((DEFAULTS["WIDTH"], DEFAULTS["HEIGHT"]),
        ...
        fPath = Path("outputs/apha/dataset.txt")
        if fPath.exists():
            fp = open("outputs/apha/dataset.txt", "a")
        else:
            fp = open("outputs/apha/dataset.txt", "w")
