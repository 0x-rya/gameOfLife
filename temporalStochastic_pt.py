...
import temporalStochastic_gol as gol
from itertools import combinations
...
def plot_phase_transitions_with_bump(data: list[float], lmbda_scale: list[float], transitions: list, rule: str, lower_threshold = 0.05, upper_threshold = 0.95, min_bump = 0.2):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"outputs/temporal-stochastic/{rule.replace('/', '-')}/plot")
    # plt.show()
    plt.close()
...
if __name__ == "__main__":
    ...
    while len(sample_pairs) > 0:
        ...
        dPath = Path(f"outputs/temporal-stochastic/{rule_str}/")
        ...
        # Write to dataset
        fPath = Path("outputs/temporal-stochastic/dataset.txt")
